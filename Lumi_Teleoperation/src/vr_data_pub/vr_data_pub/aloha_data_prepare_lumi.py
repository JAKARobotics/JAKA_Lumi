...
class DataCollector(Node):
    def __init__(self):
        ...
        # self.camera_names = ['right_cam', 'front_cam', 'left_cam']
        self.camera_names = ['right_cam', 'front_cam']
        self.data_dict1 = {
            '/observations/qpos_p': [],
            '/observations/qpos': [],
            '/observations/qvel': [],
        ...
        self.ts = message_filters.ApproximateTimeSynchronizer(
            [  # self.right_fixed_cam_sub,
                #self.left_arm_qpos_j, 
                self.right_arm_qpos_j,
                # self.left_arm_qpos_p, 
                self.right_arm_qpos_p,
                # self.left_gripper_qpos, 
                self.right_gripper_qpos,
                # self.left_arm_qvel, 
                self.right_arm_qvel,
                # self.left_vr_servo_p_pose, 
                self.right_vr_servo_p_pose,
                # self.left_vr_servo_j_pose, 
                # self.right_vr_servo_j_pose,
                # self.left_sensor_val, 
                # self.right_sensor_val,
                # self.left_vr_gripper, 
                self.right_vr_gripper,
                self.right_cam_sub, 
                self.front_cam_sub, 
                # self.left_cam_sub
                # self.head_cam_sub
    ...
    def sync_callback(self, 
                    #   left_arm_qpos_j_msg, 
                      right_arm_qpos_j_msg,
                    #   left_arm_qpos_p_msg, 
                      right_arm_qpos_p_msg,
                    #   left_gripper_msg, 
                      right_gripper_msg,
                    #   left_arm_qvel_msg, 
                      right_arm_qvel_msg,
        ...
        if self.recording:
            ...
            self.process_qpos(#left_arm_qpos_j_msg, 
                              right_arm_qpos_j_msg,
                            #   left_arm_qpos_p_msg, 
                              right_arm_qpos_p_msg,
                            #   left_sensor_msg, 
            ...
            self.process_qvel(#left_arm_qvel_msg, 
                              right_arm_qvel_msg,
                            #   left_gripper_msg, 
                              right_gripper_msg, 
                              current_dict
                              )
            self.process_action(#left_arm_qpos_j_msg, 
                                right_arm_qpos_j_msg,
                                # left_vr_pose_p_msg, 
                                right_vr_pose_p_msg,
                                # left_vr_pose_j_msg, right_vr_pose_j_msg,
            ...
            self.process_images(right_cam_msg, 
                                front_cam_msg, 
                                # left_cam_msg,
    ...
    def process_action(self, 
        ...
        # left_sensor = left_sensor_msg[:6]
        # right_sensor = right_sensor_msg[:6]
        ...
        action_p = np.concatenate([right_vr_pose_p, np.array([right_vr_gripper])])
        ...
        right_vr_pose_j = right_arm_qpos_j_msg.position[:6]
        action2 = np.concatenate([right_vr_pose_j, np.array([right_vr_gripper])])
    ...
    def process_images(self, right_cam_msg, front_cam_msg, 
                       #left_cam_msg, 
                       current_dict
                       ):
        for cam_msg, cam_name in zip([right_cam_msg, front_cam_msg, 
                                      #left_cam_msg
                                    #   head_cam_msg
                                      ], self.camera_names):
    ...
    def save_hdf5(self, current_dict):
        ...
        max_timesteps = len(current_dict['/observations/images/right_cam'])
        print(f"Saving {max_timesteps} timesteps to {dataset_path}.hdf5")
        with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
            obs = root.create_group('observations')
            qpos_p = obs.create_dataset('qpos_p', (max_timesteps, 7),
                                        data=current_dict['/observations/qpos_p'])  # 6 + 6 + 1 + 1
            qpos_j = obs.create_dataset('qpos', (max_timesteps, 7),
                                        data=current_dict['/observations/qpos'])  # 7 + 7 + 1 + 1
            qvel = obs.create_dataset('qvel', (max_timesteps, 7), data=current_dict['/observations/qvel'])  # 16
            action_p = root.create_dataset('action_p', (max_timesteps, 7),
                                           data=current_dict['/action_p'])  # 6 + 6 + 1 + 1
            action_j = root.create_dataset('action', (max_timesteps, 7),
                                           data=current_dict['/action'])  # 7 + 7 + 1 + 1
