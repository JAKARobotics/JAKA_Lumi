...
#                # 处理成功的逆解响应...
    ...
    def vr_left_pose_callback(self, msg):
        ...
        if self.vr_robot_pose_converter_status==True:
            ...
            temp_var = len(self.cur_joint_val)
            self.get_logger().warn(f"len joint val {temp_var}")
            temp_var = len(self.cur_tcp_position)
            self.get_logger().warn(f"len tcp_position {temp_var}")
            temp_var = len(self.cur_tcp_rotm)
            self.get_logger().warn(f"len tcp_rotm {temp_var}")
           # self.get_logger().warn("%d", len(self.cur_tcp_position))
           # self.get_logger().warn("%d", len(self.cur_tcp_rotm))
